...
class User(Base):
    ...
    # ✅ ADD THIS: Relationship to Conversion (one-to-many)
...
class ApiKey(Base):
    ...
    # ❌ REMOVE THIS: The Conversion relationship should be on User, not ApiKey
    # conversions = relationship("Conversion", back_populates="user", cascade="all, delete-orphan")

    # If you want ApiKey to have access to conversions, use a property inst
    @property
    def conversions(self):
        """Get conversions through the user relationship."""
        if self.user:
            return self.user.conversions
        return []
